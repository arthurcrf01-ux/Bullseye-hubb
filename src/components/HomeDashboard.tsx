...
export function HomeDashboard({ onAddClick, onNavigate }: { onAddClick: () => void; onNavigate: (view: string) => void }) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* Banner Copa 2026 Verde Original */}
      <motion.div 
      ...
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/30 shadow-[0_0_15px_rgba(255,255,255,0.2)]">
            ...
            <div>
              ...
              <p className="text-emerald-50 text-sm font-medium opacity-90">Complete seu álbum! Dicas, curiosidades e point de troca das figurinhas limitadas!</p>
            </div>
          </div>
          <button className="bg-white text-emerald-950 px-6 py-3 rounded-xl font-black uppercase tracking-wide shadow-lg">Ver Especial</button>
        </div>
      </motion.div>

      {/* Grid de Pontos Original */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-8 rounded-2xl bg-slate-900 border border-slate-800">
          ...
          <p className="text-[10px] text-slate-500 uppercase mt-2">+10 pts por item</p>
        </div>
        <div className="p-8 rounded-2xl bg-slate-900 border border-slate-800">
          ...
          <p className="text-[10px] text-slate-500 uppercase mt-2">na sua coleção virtual</p>
        </div>
        <div className="p-6 rounded-2xl bg-gradient-to-r from-indigo-900/40 to-slate-900 border border-indigo-500/30 flex items-center gap-4">
          <img src={topMonthly.avatarUrl} alt="Avatar" className="w-12 h-12 rounded-full border-2 border-indigo-400 shadow-lg" />
          <div>
